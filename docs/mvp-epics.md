...
- [Confluence Page Tree Pull Implementation Plan](superpowers/plans/2026-04-27-confluence-page-tree-pull.md)
...
- 최근 Pull 상태는 `logs/latest.md`를 읽어 요약한다.
...
- 마지막 Pull 시각을 표시한다.
- Pull Tree 버튼을 제공한다.
- Force Pull Tree 버튼을 제공하고, 승인 시 로컬 수정 파일을 원격 본문으로 덮어쓴다.
- Force Pull Tree 취소 시 변경된 로컬 파일 목록 리포트를 연다.
...
- [Current Page Push Implementation Plan](superpowers/plans/2026-04-29-current-page-push.md)

## Epic 8 확장. 현재 Markdown 문서 Pull

### 목표

현재 열려 있는 Markdown 파일 1개를 원격 Confluence 최신 본문으로 갱신한다.

### 완료 기준

- Pull Current Page 버튼과 명령을 제공한다.
- 현재 열린 Markdown 파일의 frontmatter에서 pageId, version, content hash를 읽는다.
- metadata가 없는 파일은 갱신을 차단한다.
- 원격 최신 version과 storage 본문을 조회한다.
- 원격 storage 본문을 Markdown으로 변환한다.
- 로컬 본문 hash가 frontmatter content hash와 다르면 로컬 수정본으로 판단한다.
- 로컬 수정본이 있으면 metadata가 제거된 백업본을 생성한다.
- 백업본에는 Confluence와 연결되지 않은 파일이라는 안내문을 남긴다.
- 현재 파일은 원격 최신 Markdown과 최신 frontmatter로 덮어쓴다.
- 완료 Notice에 갱신 version과 백업본 생성 여부를 표시한다.
- Force Pull Tree는 백업본을 생성하지 않고, 승인 시 백업 없이 원격 본문으로 덮어쓴다.

### 구현 계획

- [Current Page Pull Implementation Plan](superpowers/plans/2026-04-29-current-page-pull.md)
...
- [Error Handling User Safety Implementation Plan](superpowers/plans/2026-04-29-error-handling-user-safety.md)

## Epic 10. Public GitHub 기반 Vault Template 배포와 플러그인 업데이트

### 목표

비개발자를 포함한 사용자가 Public GitHub에서 vault template을 내려받아 바로 시작하고, 플러그인 안에서 새 버전을 확인하고 업데이트할 수 있게 한다.

### 완료 기준

- 순수 플러그인 GitHub 저장소 배포 구조를 제공한다.
- 플러그인이 포함된 vault template GitHub 저장소 배포 구조를 제공한다.
- 순수 플러그인 저장소와 vault template 저장소는 별도 Git 저장소로 관리한다.
- vault template 저장소 루트는 사용자가 그대로 Obsidian vault로 열 수 있는 폴더 구조여야 하며, 플러그인 저장소 안의 `vault-template/` 하위 폴더를 중첩된 형태로 배포하지 않는다.
- 로컬 개발 작업공간은 순수 플러그인 저장소에 연결하고, vault template 배포 작업공간은 별도 로컬 폴더에서 vault template 저장소에 연결한다.
- vault template에는 `.obsidian/plugins/confluence-obsidian-sync/` 산출물 구성을 포함한다.
- `.confluence-sync/` 예시 설정을 제공한다.
- `.env`와 `.env.example` 없이 플러그인 설정 UI에서 Confluence base URL, 이메일, API token을 입력할 수 있다.
- `confluence/` 산출물 폴더를 Git에서 제외한다.
- `logs/`, 개인 설정, 프로젝트 상태, 인증 정보를 Git에서 제외한다.
- 사용자가 vault template ZIP 또는 Git clone으로 받은 폴더를 Obsidian vault로 열면 바로 시작할 수 있다.
- 플러그인 설정 또는 Sync Panel에서 `Check for update`를 실행할 수 있다.
- `Update plugin`은 Public GitHub Release 산출물의 `main.js`, `manifest.json`, `styles.css`만 다운로드해 교체한다.
- 업데이트 전 사용자 확인, checksum 검증, 기존 파일 백업, 실패 시 복구를 제공한다.
- 업데이트 후 Obsidian 재시작 또는 플러그인 reload 필요 여부를 사용자에게 안내한다.
...
## Epic 12. graphify CLI 선택 연동

### 목표

graphify가 설치된 Desktop Obsidian 환경에서 Confluence 로컬 Markdown 작업 사본을 지식 그래프로 분석할 수 있는 실행 버튼을 Sync Panel에 제공한다.

### 완료 기준

- Desktop Obsidian 환경에서만 graphify 연동 영역을 표시한다.
- `graphify` 실행 파일 또는 사용자가 설정한 graphify 실행 경로를 감지한다.
- graphify가 설치되어 있지 않으면 실행 버튼 대신 설치 안내와 경로 설정 안내를 표시한다.
- graphify가 설치되어 있으면 현재 프로젝트의 Confluence 산출물 폴더를 대상으로 실행 버튼을 제공한다.
- 실행 중 상태, 성공, 실패를 Sync Panel에 표시한다.
- 실패 시 원인을 사용자가 확인할 수 있도록 오류 메시지를 남긴다.
- 실행 완료 후 `graphify-out/GRAPH_REPORT.md`, `graph.json`, `graph.html` 결과 파일을 열 수 있는 버튼을 제공한다.
- graphify 자체를 플러그인 번들에 포함하지 않는다.
- 자체 AI/RAG UI나 자체 지식 그래프 엔진은 만들지 않는다.
- Mobile Obsidian에서는 graphify 실행을 지원하지 않는다.

### 구현 계획

- [Graphify CLI Integration Implementation Plan](superpowers/plans/2026-04-29-graphify-cli-integration.md)
...
### Milestone 4. Public GitHub 배포 가능 MVP

Epic 9부터 Epic 11까지 완료한다.

사용자는 Public GitHub에서 vault template을 ZIP으로 내려받거나 clone하고, Obsidian에서 열어 MVP 기능을 사용할 수 있다.
